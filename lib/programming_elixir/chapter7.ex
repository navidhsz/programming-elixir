defmodule ProgrammingElixir.Chapter7 do
  ...
  defp maxHelper([head | tail], currentValue) when head != nil and head >= currentValue,
    do: maxHelper(tail, head)

  defp maxHelper([head | tail], currentValue) when head != nil and head < currentValue,
    do: maxHelper(tail, currentValue)
  ...
  defp caesarHelper([head | tail], n) when head + n > ?z,
    do: [head + n - ?z + ?a - 1 | caesarHelper(tail, n)]
